--
-- XML library tests
-- Source: https://github.com/lunarmodules/Penlight/blob/master/tests/test-xml.lua
-- 2021-05-07
--
return function(xmlLib)
local xml = xmlLib

local function asserteq(v1, v2)
	if type(v1) ~= type(v2) then
		errorf("Type!\n>>>>>>>>>>>>>>>>\n%s\n------ vs ------\n%s\n<<<<<<<<<<<<<<<<", type(v1), type(v2))
	end
	if type(v1) == "table" then
		for k in pairs(v1) do  asserteq(v1[k], v2[k])  end
		for k in pairs(v2) do  asserteq(v1[k], v2[k])  end
	else
		if v1 ~= v2 then
...
local function match(t,expect)
    local res,ret = d:match(t)
    asserteq(res,expect,0,1) ---> note extra level, so we report on calls to this function!
end
