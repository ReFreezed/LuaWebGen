...
--[[------------------------------------------------------
	...
	--
	Edited by Marcus Thunström, 2018-07-04
----------------------------------------------------------
	date( [ isNow=false ] )  -> dateObject  -- If isNow is false, then UTC time is used.
	date( dateString )  -> dateObject
	date( dateTable )  -> dateObject
	date( time )  -> dateObject
	date( year, month, day [, hours, minutes, seconds, tick ] )  -> dateObject
	date.diff( datetime:* )  -> dateObject
	date.epoch( )  -> dateObject
	date.isdateobj( value )  -> bool
	date.isleapyear( year )  -> bool
	date.isodate( year [, week, weekday ] )  -> dateObject
	date.time( [ hours=0, minutes=0, seconds=0, tick=0 ])  -> dateObject
	date.version  -> number  -- V_MMM_RRRR; V=major, M=minor, R=revision
	dateObject:__add( datetime:* )  -> dateObject
	dateObject:__concat( value )  -> string
	dateObject:__eq( dateObject )  -> bool
	dateObject:__le( dateObject )  -> bool
	dateObject:__lt( dateObject )  -> bool
	dateObject:__sub( datetime:* )  -> dateObject
	dateObject:__tostring( )  -> dateObject:fmt( )
	dateObject:adddays( days )  -> self
	dateObject:addhours( hours )  -> self
	dateObject:addminutes( minutes )  -> self
	dateObject:addmonths( months [, days=0 ] )  -> self
	dateObject:addseconds( seconds )  -> self
	dateObject:addticks( ticks )  -> self
	dateObject:addyears( years [, months=0, days=0 ] )  -> self
	dateObject:copy( )  -> dateObject
	dateObject:fmt( format )  -> string
	dateObject:fmt0( format )  -> string
	dateObject:getbias( )  -> timezoneOffset
	dateObject:getclockhour()  -> clockHour  -- 1..12
	dateObject:getdate( )  -> year, month, day
	dateObject:getday( )  -> day
	dateObject:getfracsec( )  -> seconds:float
	dateObject:gethours( )  -> hours
	dateObject:getisodate( )  -> year, week, weekday
	dateObject:getisoweekday( )  -> weekday
	dateObject:getisoweeknumber( )  -> weekNumber
	dateObject:getisoyear( )  -> year
	dateObject:getminutes( )  -> minutes
	dateObject:getmonth( )  -> month
	dateObject:getseconds( )  -> seconds
	dateObject:getticks( u? )  -> ticks
	dateObject:gettime( )  -> time
	dateObject:gettzname( )  -> timezoneName
	dateObject:getweekday( )  -> weekday
	dateObject:getweeknumber( [ startingWeekday=1 ] )  -> weekNumber
	dateObject:getyear( )  -> year
	dateObject:getyearday( )  -> yearDay  -- 1..366
	dateObject:normalize( )  -> self
	dateObject:setday( day )  -> self
	dateObject:sethours( hours [, minutes=keep, seconds=keep, ticks=keep ] )  -> self
	dateObject:setisoweekday( weekday )  -> self
	dateObject:setisoweeknumber( weekNumber [, weekday=keep ] )  -> self
	dateObject:setisoyear( year [, week=keep, weekday=keep ] )  -> self
	dateObject:setminutes( minutes [, seconds=keep, ticks=keep ] )  -> self
	dateObject:setmonth( month [, day=keep ] )  -> self
	dateObject:setseconds( seconds [, ticks=keep ] )  -> self
	dateObject:setticks( ticks )  -> self
	dateObject:setyear( year [, month=keep, day=keep ] )  -> self
	dateObject:spandays( )  -> days
	dateObject:spanhours( )  -> hours
	dateObject:spanminutes( )  -> minutes
	dateObject:spanseconds( )  -> seconds
	dateObject:spanticks( )  -> ticks
	dateObject:tolocal( )  -> self
	dateObject:toutc( )  -> self
	--
	Note: 'month' arguments can be number (1..12) or month name (english, long or short format).
	Note: 'weekday' is: 1=sun, 2=mon, ... 7=sat
--]]------------------------------------------------------
--[[ CONSTANTS ]]--
	local HOURPERDAY   = 24
	local MINPERHOUR   = 60
...
--[[ DATE FUNCTIONS ]]--
	...
	local function setticks(t)
		TICKSPERSEC  = t
		TICKSPERDAY  = TICKSPERSEC*SECPERDAY
		TICKSPERHOUR = TICKSPERSEC*SECPERHOUR
		TICKSPERMIN  = TICKSPERSEC*SECPERMIN
	end
	-- is year y leap year?
...
--[[ THE DATE OBJECT ]]--
	...
	function strwalker:__call(s, f)
		-- print("strwalker:__call "..s..self:aimchr())
	...
	local function date_parse(str)
		...
		if not (
			sw("^(%d%d%d%d)",sety)
			and (
				sw("^(%-?)(%d%d)%1(%d%d)", function(_,a,b) setm(tonumber(a)); setd(tonumber(b)) end)
				or sw("^(%-?)[Ww](%d%d)%1(%d?)", function(_,a,b) w, u = tonumber(a), tonumber(b or 1) end)
				or sw("^%-?(%d%d%d)", setj)
				or sw("^%-?(%d%d)", function(a) setm(a); setd(1) end)
			)
			and (
				(
					sw("^%s*[Tt]?(%d%d):?", seth)
...
--[[ THE LOCAL DATE OBJECT METHODS ]]--
	...
	function date.isdateobj(v) return not not date_isdobj(v) end
